<template>
  <modal id="podcasts-edit-modal" title="Edit podcast" widthAuto="true">
  ...
  methods: {
    async read () {
      try {
        ...
        this.podcast = await this.$request.podcasts().read(this.podcastId)
        this.podcast.thumbnail = ''
        this.podcast.audio = {}
      } catch (error) {
    ...
    async edit () {
      try {
        ...
        await this.$request.podcasts().edit(this.podcast)
        this.$modal.podcasts().close('edit')
    ...
    onClose () {
      this.$modal.podcasts().onClose('edit', () => {
        this.clear()
      })
    },
