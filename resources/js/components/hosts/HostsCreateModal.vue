<template>
  <modal id="hosts-create-modal" title="Add host" widthAuto="true">
  ...
  methods: {
    async create () {
      try {
        ...
        await this.$request.hosts().create(this.host)
        this.$modal.hosts().close('create')
    ...
    onClose () {
      this.$modal.hosts().onClose('create', () => {
        this.clear()
      })
    },
    updateAvatar (data) {
