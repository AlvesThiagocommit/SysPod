<template>
  <div class="row hosts-header">
    <div class="col">
      <div class="d-flex justify-content-between">
        <primary-button title="Create" icon="add-circle" @click.native="create()"></primary-button>
        <primary-button
          title="Trash"
          icon="trash"
          class="bg-light text-secondary border-light"
          v-if="active === 'paginate'"
          @click.native="activate('trash')">
        </primary-button>
        <primary-button
          icon="arrow-round-back"
          title="Go back"
          v-else
          @click.native="activate('paginate')">
        </primary-button>
      </div>
    </div>
    <hosts-create-modal></hosts-create-modal>
  </div>
</template>

<script>
import PrimaryButton from '../PrimaryButton'
import HostsCreateModal from './HostsCreateModal'

export default {
  components: {
    PrimaryButton,
    HostsCreateModal
  },

  computed: {
    active () {
      return this.$store.hosts().active().collect()
    }
  },

  methods: {
    activate (active) {
      this.$store.hosts().active().replace(active)
    },
    create () {
      this.$modal.hosts().open('create')
    }
  }
}
</script>

<style lang="sass" scoped>
.hosts-header
  margin-bottom: 70px
</style>
